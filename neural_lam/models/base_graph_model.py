...
class BaseGraphModel(ARModel):
    ...
    def predict_step(self, prev_state, prev_prev_state):
        ...
        prev_prev_state = prev_prev_state.to(self.device)
        ...
        # Embedd all features
        grid_emb = self.grid_embedder(grid_features).to(self.device)  # (B, N_grid, d_h)
        g2m_emb = self.g2m_embedder(self.g2m_features).to(self.device)  # (M_g2m, d_h)
        m2g_emb = self.m2g_embedder(self.m2g_features).to(self.device)  # (M_m2g, d_h)
        mesh_emb = self.embedd_mesh_nodes().to(self.device)

        # Map from grid to mesh
        mesh_emb_expanded = self.expand_to_batch(mesh_emb, batch_size).to(self.device)
        node_rep = torch.cat(
            (mesh_emb_expanded, grid_emb),
            dim=1).to(
            self.device)  # (B, N, d_h)
        g2m_emb_expanded = self.expand_to_batch(g2m_emb, batch_size).to(self.device)
